...
hikeBox = document.getElementById("hike-container");

function getTags(tags){
  let res = ``;
  tags.forEach(tag => {
    res += `<button>${tag}</button>`;
  });
  return res;
}
const difs = [
  `<span class="rating" role="img" aria-label="1 out of 5">🥾⬜⬜⬜⬜</span>`,
  `<span class="rating" role="img" aria-label="2 out of 5">🥾🥾⬜⬜⬜</span>`,
  `<span class="rating" role="img" aria-label="3 out of 5">🥾🥾🥾⬜⬜</span>`,
  `<span class="rating" role="img" aria-label="4 out of 5">🥾🥾🥾🥾⬜</span>`,
  `<span class="rating" role="img" aria-label="5 out of 5">🥾🥾🥾🥾🥾</span>`
];

function drawHike(hike){
  hikeBox.innerHTML += `
    <div class="hike-card">
...
document.querySelector("button").addEventListener("click", function() {
  ...
  let filteredHikes = sortedHikes.filter(function (h){
  return (
    h.name.toLowerCase().includes(search.toLowerCase()) ||
    h.description.toLowerCase().includes(search.toLowerCase()) ||
    h.tags.find(t => t.toLowerCase().includes(search.toLowerCase()))
  );});
...
drawHike(hikes[Math.floor(Math.random()*hikes.length)]);