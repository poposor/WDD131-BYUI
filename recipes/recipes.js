...
loadRecipe(recipes[Math.floor(Math.random() * recipes.length)]);
...
document.querySelector("#search button").addEventListener("click", () => {
    ...
    let = filteredRecipes = recipes.filter(function (r){
        return (
            ...
            r.tags.find(t => t.toLowerCase().includes(search.toLowerCase()))
        );
    });

    let result = filteredRecipes.sort(compareNames);
